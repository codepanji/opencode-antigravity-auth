...
function removeUnsupportedKeywords(schema, isInsideProperties = false) {
    if (!schema || typeof schema !== "object") {
        return schema;
    }
    if (Array.isArray(schema)) {
    ...
    const result = {};
    for (const [key, value] of Object.entries(schema)) {
        if (!isInsideProperties && UNSUPPORTED_KEYWORDS.includes(key)) {
            continue;
        }
        ...
        if (sessionId && getCachedSignatureFn) {
            ...
            if (text) {
                ...
                if (cachedSignature && cachedSignature.length >= 50) {
                    ...
                    filtered.push(sanitizeThinkingPart(restoredPart));
                    continue;
                }
            }
        }
 ...
 */
export function recursivelyParseJsonStrings(obj) {
    if (obj === null || obj === undefined) {
        return obj;
    }
    if (Array.isArray(obj)) {
